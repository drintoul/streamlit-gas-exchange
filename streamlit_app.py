...
def main():
	...
	if cdn:
		...
		amount = st.slider("Enter Amount of gas in Litres", 1.0, 40.0, 10.0)
		...
		amount = st.slider("Enter Amount of gas in gallons", 1.0, 15.0, 10.0)
	...
	st.info(f"{amount} {sUnits}s at \${price:.2f}/{sUnits} {sCurrency} with an exchange rate of \${rate:.2f} would cost \${cost:.2f} {tCurrency}")
