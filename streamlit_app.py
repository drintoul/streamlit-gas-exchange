...
def main():
	...
	if cdn:
		rate = cdn2usd
		st.write(f"Today's Exchange rate is \$CDN {rate} = \$USD 1.00")
		...
		rate = usd2cdn
		st.write(f"Today's Exchange rate is \$CDN 1.00 = \$USD {rate}")
	...
	st.info(f"{amount} {sUnits}s at \${price:.2f}/{sUnits} and an exchange rate of \${rate:.2f}/{sCurrency} to {tCurrency} would cost \${cost:.2f} {tCurrency} dollars")
